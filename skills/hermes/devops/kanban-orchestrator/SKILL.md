...
### Protocol violation crash loop (rc=0 without kanban_complete)

A common crash loop: the worker process exits cleanly (exit_code=0) but **never calls `kanban_complete` or `kanban_block`**. The dispatcher reports:

```
worker exited cleanly (rc=0) without calling kanban_complete or kanban_block — protocol violation
```

The worker may retry multiple times, each retry crashing the same way, until the max_retry limit is hit and the task goes `blocked`.

**Root cause**: The worker profile's SOUL.md (or project context file) lacks kanban protocol guidance. The LLM picks up the task but has no instruction to call `kanban_complete`, so it exits after producing output.

**Diagnosis**: Run `hermes kanban show <task_id> --json` and look for `protocol_violation` events in the event log. If you see repeated `crashed` runs all with the same error, this is the issue.

**Fix**: Add kanban worker guidance to the profile's SOUL.md. The key content the worker is missing:
- Must call `kanban_complete(summary=...)` when work is done
- May use `kanban_comment(body=...)` for context
- May use `kanban_block(reason=...)` if blocked
- Use `kanban_*` **tools**, not `hermes kanban` CLI (CLI won't work inside Docker/containerized workers)

**Fast path**: If the profile doesn't need fixing, `reassign` the task to a working profile instead.
