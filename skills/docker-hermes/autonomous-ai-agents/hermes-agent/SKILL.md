...
To manually run a gateway for a specific profile (especially if s6 service registration is bypassed or fails), prefix the command with the explicit `HERMES_HOME` path for that profile instead of relying solely on `--profile` or `HERMES_PROFILE`.

```bash
# Good: Forces the gateway to load the correct config and not conflict with the default instance
HERMES_HOME=/opt/data/hermes/profiles/<name> /opt/hermes/.venv/bin/hermes gateway run --no-supervise --accept-hooks
```
hermes gateway run --no-supervise  Start gateway directly (bypasses s6 supervisor in containers)
...
**Profile Isolation Caveats:**
- **Feishu WebSockets**: A single Feishu App ID cannot be connected via WebSocket by multiple running Hermes gateway processes simultaneously. If the `default` gateway and a custom profile gateway both use the same Feishu credentials in `websocket` mode, one will crash with a connection conflict error.
- **`--replace` and PID locks**: When manually starting a profile gateway using `hermes --profile <name> gateway run --replace`, the process checks for `gateway.pid` in `$HERMES_HOME`. If the command fails to scope `$HERMES_HOME` correctly, it may mistakenly detect the `default` gateway's PID, attempt to replace it, or refuse to start ("Another gateway instance is already running"). Prepend `HERMES_HOME=/path/to/profile/dir` to isolate it safely.
...
### Using Host-Installed CLI Tools in Container

If a CLI tool is installed on the host but not inside the Hermes container, the simplest approach is to install it inside the container (e.g., `npm install -g @larksuite/cli` for lark-cli). Mounting host binaries is fragile. For tools with MCP support, prefer MCP server integration — see `native-mcp` skill and `productivity/lark-cli` for examples.
...
Skills ship from two locations — user-local (`~/.hermes/skills/`) created via `skill_manage(action='create')`, and in-repo (`skills/<category>/<name>/SKILL.md`) committed to the hermes-agent repo. This section covers in-repo skills.

**Required frontmatter** (validated by `tools/skill_manager_tool.py`):
- Starts with `---` at byte 0, closes with `\n---\n`
- `name` (lowercase, hyphens, ≤64 chars), `description` (≤1024 chars), non-empty body
- Convention: add `version`, `author`, `license`, `metadata.hermes.{tags, related_skills}`

**Target size:** 8–15k chars. Push past 20k → split into `references/*.md`.

**Structure:**
```
# Title → ## Overview → ## When to Use → <body sections> → ## Common Pitfalls → ## Verification Checklist
```

**Workflow:**
1. Survey 2-3 peers in target category
2. Draft with `write_file` to `skills/<category>/<name>/SKILL.md`
3. Validate: frontmatter parses, name/description present, size OK
4. `git add` + commit

**Pitfalls:**
- `skill_manage(action='create')` writes to `~/.hermes/skills/`, NOT the repo tree — use `write_file` for in-repo
- Skill loader is cached at session start — new skills invisible until fresh session
- `related_skills` referencing user-local skills breaks for other clones
