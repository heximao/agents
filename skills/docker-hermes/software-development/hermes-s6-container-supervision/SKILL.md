...
### "no such gateway 'X'" Error When Starting Profile Gateways
In container environments where the service manager runs as `s6` (supervised), starting a gateway for a specific profile via `hermes -p X gateway start` or `hermes -p X gateway restart` can sometimes yield a "no such gateway 'X'" error (e.g. `GatewayNotRegisteredError`). 
This occurs because s6 expects profile gateways to be explicitly registered in its dynamic scan directory first, but manual `--replace` or `start` commands sometimes race or bypass the registration step.

**Fix/Workaround 1: Force manual foreground run (No S6 supervision)**
If you just want to run the gateway in foreground mode to bypass s6 supervision entirely (useful when Python command execution is heavily restricted), you MUST use `--no-supervise` to prevent the CLI from recursively redirecting your command back into the s6 supervisor:
```bash
HERMES_PROFILE=<name> hermes gateway run --no-supervise --replace --accept-hooks
```
*Note:* Use `terminal(background=true, notify_on_complete=true)` when invoking this workaround so it stays alive.

**Fix/Workaround 2: Manual S6 Registration (Recommended to keep supervision)**
If direct Python integration scripts are blocked (e.g., security sandbox), you can manually seed the service directory in `/run/service/` (the dynamic scandir) to register the profile gateway under S6 supervision:

1. Create the service directory structure:
```bash
mkdir -p /run/service/gateway-<name>/log
```

2. Write the service type (`longrun`):
```bash
echo "longrun" > /run/service/gateway-<name>/type
```

3. Write the service run script `/run/service/gateway-<name>/run`:
```sh
...
exec s6-setuidgid hermes hermes -p <name> gateway run
```

4. Write the logging run script `/run/service/gateway-<name>/log/run`:
```sh
#!/command/with-contenv sh
# shellcheck shell=sh
: "${HERMES_HOME:=/opt/data}"
log_dir="$HERMES_HOME/logs/gateways/<name>"
mkdir -p "$log_dir"
chown -R hermes:hermes "$log_dir" 2>/dev/null || true
exec s6-setuidgid hermes s6-log 1 n10 s1000000 T "$log_dir"
```

5. Set executable permissions on the run scripts:
```bash
chmod +x /run/service/gateway-<name>/run /run/service/gateway-<name>/log/run
```

6. Pre-create empty directories for `supervise` to avoid permission races:
```bash
mkdir -p /run/service/gateway-<name>/supervise /run/service/gateway-<name>/log/supervise
```

7. Trigger an S6 rescan and start the service:
```bash
