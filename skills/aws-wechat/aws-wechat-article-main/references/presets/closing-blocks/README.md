...
本目录下的 `.md` 文件为**文末固定引导区块**。写作时按 config 的 `custom_closing_block` > `default_closing_block` 或 config 中的 `closing_block` 字符串加载。
...
- 文件名即预设名（不含后缀），如 `brand.md` → 预设名 `brand`。在 config 中设置 `default_closing_block` 或 `custom_closing_block`（`custom_*` 优先）即可使用。
