# Excalidraw — Full Element Reference
...
> use the container binding approach.
...
- Text is auto-centered when `containerId` is set
- The text `x`/`y`/`width`/`height` are approximate — Excalidraw recalculates on load
