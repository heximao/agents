...
- `outcome: "crashed"` — OOM or segfault. Reduce memory footprint. Also check for "protocol violation" — if all crashed runs say "worker exited cleanly (rc=0) without calling kanban_complete", the SOUL.md lacks kanban guidance. The orchestrator must fix the SOUL.md or reassign to a working profile.
